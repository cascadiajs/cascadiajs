...
export async function get({ pathParameters, query, path }) {
  ...
  if (!talk) {
    ...
    const location = createImageUrl({
      profileImage: `https://cascadiajs.com${talk.speaker.image}`,
      fullName: talk.speaker.name,
    });
    return {
      location,
    };
  ...
  const sharing = {
    ...
    sharingTitle: "CascadiaJS 2026 | Speakers | " + talk.speaker.name,
    //sharingImage: '/_public/images/sharing' + path + '.png',
    sharingImage: `/_public/images/2026/social/speaker-${talk.speaker.slug}.png`,
    //sharingImage: talk.thumbnail || `/2026/talks/${talk.slug}?image`,
